...
int btSize = 100; boolean test1 = false;
int GMtick = 0;

//array area
int[] iList1 = new int[26];
float[] fList1 = new float[26];
String[] sList1 = {"Jan", "Harry", "Larry", "Jack", "Stanley", "Dennis", "Omar", "Henry", "Ford", "Sans", "Toby", "Gaster"};
String[] sList2 = new String[5];
...
void runCode() {
  for(int i = 0; i < sList2.length; i++){
   int holder = sList1.length;
    holder = (int) random(0, holder);
    sList2[i] = sList1[holder];
    test1 = sList1[holder] == "Jan";
  }
  println(sList2);
  println(test1);
